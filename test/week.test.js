...
group('성장 경로', () => {
  ...
  eq(s.map(x => x.w), [12,20,28,40],        '현재 주차가 고정 칸이면 칸이 늘지 않는다');
  eq(s.filter(x => x.now).map(x => x.w),  [20],    '지금 칸은 현재 주차 하나뿐');
  eq(s.filter(x => x.past).map(x => x.w), [12],    '지나온 칸은 현재 주차 앞의 것뿐');
  eq(s.filter(x => x.last).map(x => x.w), [40],    '마지막 칸은 40주 하나');
  eq(s[0].key, '2026-10-15',                       '12주 칸의 날짜는 12주 0일 (LMP+84)');
  eq(s[3].key, F.ymd(EDD),                         '40주 칸의 날짜는 예정일');

  /* 🔴 칸이 다섯을 넘으면 폰 너비(좁으면 316px)에 안 들어간다. 고정 넷 + 끼어드는 하나가 상한이다 */
  eq(s.length <= 4, true,                          '고정 칸은 넷');
  for (let i = 0; i <= 287; i++) {
    eq(F.growthStops(LMP, F.addDays(LMP, i)).length <= 5, true, i + '일째에 칸이 다섯 이하');
  }

  /* 2026-12-24 = LMP+154일 = 22주 0일. 고정 칸이 아니라 사이에 끼어든다 */
  const t = F.growthStops(LMP, D('2026-12-24'));
  eq(t.map(x => x.w), [12,20,22,28,40],     '현재 주차가 고정 칸이 아니면 제 자리에 끼어든다');
  eq(t.filter(x => x.now).map(x => x.w), [22],      '끼어든 칸이 지금 칸');

  /* 2026-08-06 = LMP+14일 = 2주 0일. 첫 고정 칸(12주)보다 앞이다 */
  const e = F.growthStops(LMP, D('2026-08-06'));
  eq(e.map(x => x.w), [2,12,20,28,40],      '12주 이전이면 현재 주차가 맨 앞에 온다');
  eq(e.filter(x => x.past).length, 0,               '맨 앞이면 지나온 칸이 없다');

  /* 2027-05-06 = LMP+287일 = 41주. 예정일이 지난 경우 */
  const o = F.growthStops(LMP, D('2027-05-06'));
  eq(o.map(x => x.w), [12,20,28,40],        '예정일이 지나도 40주 뒤에 칸을 만들지 않는다');
...
group('작성자 라벨', () => {
  ...
  /* 카드 제목과 입력칸 안내도 같은 이유로 1인칭을 쓰지 않는다.
